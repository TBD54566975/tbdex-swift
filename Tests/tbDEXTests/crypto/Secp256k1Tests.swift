...
final class Secp256k1Tests: XCTestCase {
  ...
  func test_bytesToPrivateKey_returnedInJwkFormat() throws {
    let privateKeyBytes = Data.fromHexString(
      "740ec69810de9ad1b8f298f1d2c0e6a52dd1e958dc2afc85764bec169c222e88")!
  ...
  func test_verify() throws {
    ...
    let isValid = try Secp256k1.verify(
      publicKey: publickey, signature: signature, signedPayload: data)
  ...
  func test_verify_returnsFalseIfSignedDataWasMutated() throws {
    ...
    let signature = try Secp256k1.sign(privateKey: privateKey, payload: data)
    var isValid = try Secp256k1.verify(
      publicKey: publickey, signature: signature, signedPayload: data)
    XCTAssertTrue(isValid)
    ...
    isValid = try Secp256k1.verify(
      publicKey: publickey, signature: signature, signedPayload: mutatedData)
  ...
  func test_verify_returnsFalseIfSignatureWasMutated() throws {
    ...
    isValid = try Secp256k1.verify(
      publicKey: publickey, signature: mutatedSignature, signedPayload: data)
    XCTAssertFalse(isValid)
  }
  ...
  func test_verify_returnsFaleWithSignatureGeneratedUsingDifferentPrivateKey() throws {
    ...
    let isValid = try Secp256k1.verify(
      publicKey: publicKeyB, signature: signature, signedPayload: data)
    XCTAssertFalse(isValid)
  }
